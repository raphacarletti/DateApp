...
open class LabelSegment: BetterSegmentedControlSegment {
    ...
    open func createLabel(withText text: String?,
        ...
        if kerning {
            label.attributedText = NSAttributedString(string: text ?? "", attributes: [.foregroundColor: textColor, .kern: 2])
        }
        return label
    }
}
...
public extension LabelSegment {
    class func segments(withTitles titles: [String],
                        ...
                        selectedFont: UIFont? = nil,
                        selectedTextColor: UIColor? = nil,
                        kerning: Bool = false) -> [BetterSegmentedControlSegment] {
        titles.map {
            LabelSegment(text: $0,
