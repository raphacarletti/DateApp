...
final class MySavedViewModel {
    private var events: [Event] = []
    private(set) var selectedEvent: Event?
    private let worker = MySavedWorker()
    ...
    func cellForRow(tableView: UITableView, indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: String(describing: EventTableViewCell.self), for: indexPath) as? EventTableViewCell, let event = events[safe: indexPath.row] else { return UITableViewCell() }
...
extension MySavedViewModel: EventSummaryViewDelegate {
